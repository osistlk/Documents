...
const data = JSON.parse(fs.readFileSync('gis/fairfax city/data.json', 'utf8'));
...
fs.writeFileSync('gis/fairfax city/parsed.json', JSON.stringify(parsedData, null, 2));

const keys = Object.keys(parsedData[0]);
header = keys.join(',');
const csv = parsedData.map(row => keys.map(key => row[key]).join(',')).join('\n');
fs.writeFileSync('gis/fairfax city/parsed.csv', header + '\n' + csv);
